...
<body>
	...
	<%
		...
		if (!userID.equals(bbs.getUserID())) { // 작성자와 수정하려는 자가 동일하지 않을 경우
			...
			if (request.getParameter("bbsTitle") == null || request.getParameter("bbsContent") == null) { // 제목과 내용이 빈칸인지?
				PrintWriter script = response.getWriter();
				script.println("<script>");
				...
				int result = bbsMgr.update(bbsID,request.getParameter("bbsTitle"), request.getParameter("bbsContent"));
				if (result == -1) {
					PrintWriter script = response.getWriter();
					script.println("<script>");
